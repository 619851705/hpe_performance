...
grep selected `find $dname -maxdepth 1 -type f`| sed "s/.*_run_\([1-9]\+\)_.*_query\(.\{1,2\}\)_sql_2017-[0-9]\+-[0-9]\+-[0-9]\+-[0-9]\+:\(.*\) row.*selected (\(.*\) seconds)/\1\t\2\t\3\t\4/" | sort -t$'\t' -nk 2.1,2.2  | awk '{q[$2][$1]=$2;r[$2][$1]=$3;t[$2][$1]=$4}END{ for (i in q) {  print q[i][1],"\t",r[i][1],"\t",t[i][1],"\t",t[i][2],"\t",t[i][3]}}' | sort -n
