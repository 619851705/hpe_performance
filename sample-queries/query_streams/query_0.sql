...
( select i_item_id item_id ,sum(ss_ext_sales_price) as ss_item_rev
 from store_sales
     ...
     JOIN (select d1.d_date
                 ...
                 where d2.d_date = '2001-01-03') sub ON date_dim.d_date = sub.d_date
...
select   
 ...
 from (
 	...
 	,w_state
 	,w_country
 	,concat('GREAT EASTERN', ',', 'AIRBORNE') as ship_carriers
       ,d_year as year
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2002
 	and t_time between 52170 and 52170+28800 
...
select  ca_zip, ca_state, sum(ws_sales_price)
 from web_sales
      JOIN customer on web_sales.ws_bill_customer_sk = c_customer_sk
      JOIN customer_address on customer.c_current_addr_sk=customer_address.ca_address_sk
      JOIN date_dim on web_sales.ws_sold_date_sk=date_dim.d_date_sk
      JOIN item on web_sales.ws_item_sk=item.i_item_sk
      where
         substr(ca_zip,1,5) in ('85669', '86197','88274','83405','86475', '85392', '85460', '80348', '81792')
         and (item.i_item_id in (select i_item_id
                             from item i2
...
select  i_item_id
 ...
 and d_date between cast('1999-06-14' as date) and (cast('1999-08-13' as date))
 and i_manufact_id in (346,645,577,595)
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Wilkinson County','Karnes County','Richland County','Levy County',
                           'Raleigh County','Maverick County','Salem County','Gage County')
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('beige','burnished','indian','coral','salmon','chocolate') and
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2)
    and store.s_county in ('Nowata County','Raleigh County','Lunenburg County','Baltimore County')
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Winchester','Shady Grove','Milton','Woodland','Mountain View') 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('misty','chocolate','metallic'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 3
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('KY', 'OR', 'IL')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('KS', 'TX', 'MO')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NC', 'SC', 'VA')
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 20 and 20+10
          or ss_coupon_amt between 8814 and 8814+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 112 and 112+10
          or ss_coupon_amt between 4269 and 4269+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 177 and 177+10
          or ss_coupon_amt between 12161 and 12161+1000
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (7,10,1,11,12,9) and
       d_year = 1999 and
       ca_state in ('SD','OH','VA'
                   ,'GA','TX','LA','NE')
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Shoes','Men','Jewelry') and
          i_class in ('kids','accessories','estate')
         )
      or (i_category in ('Music','Children','Books') and
          i_class in ('classical','infants','parenting') 
        ))
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and (household_demographics.hd_dep_count = 9 or
             household_demographics.hd_vehicle_count= 4)
        and date_dim.d_year in (2000,2000+1,2000+2)
        and store.s_city in ('Harmony','Friendship')
