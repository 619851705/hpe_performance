...
select  i_item_id
 ...
 and i_manufact_id in (473,368,675,261)
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_country = 'United States'
  and ca_state in ('OR', 'SC', 'AR')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NE', 'VA', 'MI')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-06-30','1999-09-28','1999-11-12')))
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Electronics','Women','Sports') and
          i_class in ('audio','dresses','baseball')
         )
      or (i_category in ('Men','Music','Home') and
          i_class in ('accessories','rock','tables') 
        ))
...
select  
 ...
 and d_date between (cast ('1998-02-28' as date) - 30 days)
                and (cast ('1998-02-28' as date) + 30 days) 
 group by
    w_state,i_item_id
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Forest Hills'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  62741
   and ib_upper_bound   <=  62741 + 50000
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2001-04-18' as date) 
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Huron County','Surry County','Mobile County','Lunenburg County',
                           'Daviess County','Pennington County','Arthur County','Reagan County')
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and store.s_city in ('Monroe','Mountain View')
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('sienna','orchid','navajo','medium','hot','moccasin') and
...
select  i_item_id
 ...
 and d_date between cast('2000-01-22' as date) 
 				and (cast('2000-01-22' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 30 and 30+10
          or ss_coupon_amt between 12681 and 12681+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 3 and 3+10
          or ss_coupon_amt between 13049 and 13049+1000
     ...
     (select avg(ss_list_price) B5_LP
      ...
      where ss_quantity between 21 and 25
        and (ss_list_price between 117 and 117+10
          or ss_coupon_amt between 1767 and 1767+1000
...
and d_date between '2002-03-22' and 
        (cast('2002-03-22' as date) + 90 days)
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 47866 and 47866+28800 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('chiffon','saddle','snow'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2000
 and     d_moy                   = 4
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2)
    and store.s_county in ('Warren County','Coal County','Montgomery County','Bronx County')
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1999,1999+1,1999+2) 
    and store.s_city in ('Chester','Mount Zion','Oakland','Edgewood','Springfield') 
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (10,7,8,3,4,5) and
       d_year = 1999 and
       ca_state in ('NE','OH','AL'
                   ,'UT','VA','LA','NC')
