...
if [ $? -ne 0 ]; then
	SKIP=0
	if [ -e "apache-maven-3.3.1-bin.tar.gz" ]; then
		SIZE=`du -b apache-maven-3.3.1-bin.tar.gz | cut -f 1`
		if [ $SIZE -eq 8059952 ]; then
			SKIP=1
		fi
	fi
	if [ $SKIP -ne 1 ]; then
	...
	tar -zxf apache-maven-3.3.1-bin.tar.gz > /dev/null
	CWD=$(pwd)
	export MAVEN_HOME="$CWD/apache-maven-3.3.1"
