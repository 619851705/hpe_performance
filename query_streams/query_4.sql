...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Cedar Grove'
   ...
   and ib_lower_bound   >=  55807
   and ib_upper_bound   <=  55807 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2001) and
        ((i_category in ('Sports','Books','Home') and
          i_class in ('guns','parenting','tables')
         )
      or (i_category in ('Shoes','Men','Women') and
          i_class in ('athletic','pants','maternity') 
        ))
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('ID', 'SC', 'NC')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('IN', 'AR', 'FL')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('ND', 'WY', 'IA')
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('chiffon','powder','purple','green','black','grey') and
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 49 and 49+10
          or ss_coupon_amt between 15200 and 15200+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 106 and 106+10
          or ss_coupon_amt between 12662 and 12662+1000
     ...
     (select avg(ss_list_price) B5_LP
      ...
      where ss_quantity between 21 and 25
        and (ss_list_price between 162 and 162+10
          or ss_coupon_amt between 9767 and 9767+1000
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2002
 	and t_time between 21439 and 21439+28800 
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Gogebic County','Green Lake County','Chambers County','Tehama County',
                           'Terrell County','Fairfield County','Reagan County','Wadena County')
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('floral','pink','navajo'))
...
select  i_item_id
 ...
 and i_manufact_id in (478,864,718,788)
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (3,8,6,7,2,12) and
       d_year = 2002 and
       ca_state in ('TN','TX','WY'
                   ,'SC','AR','OK','IA')
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2) 
    and store.s_city in ('Ebenezer','Richland','Highland','Grandview','Oakland') 
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2001-04-12','2001-08-18','2001-11-07')))
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Brazos County','Hubbard County','Salem County','Maverick County')
...
select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2000-03-22' as date) - 30 days)
                and (cast ('2000-03-22' as date) + 30 days) 
 group by
    w_state,i_item_id
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and (household_demographics.hd_dep_count = 6 or
             household_demographics.hd_vehicle_count= 2)
        and date_dim.d_year in (1998,1998+1,1998+2)
        and store.s_city in ('Eureka','Clinton')
...
where 
	...
	and d_date between cast('2000-06-04' as date) 
				and (cast('2000-06-04' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
and d_date between '1998-02-07' and 
        (cast('1998-02-07' as date) + 90 days)
