...
#	my $cmd="echo 'use $db->{${suite}}; source $query;' | hive -i testbench.settings 2>&1  | tee $query.log";
	my $cmd="echo 'use $db->{${suite}}; source $query;' | /opt/tez-autobuild-master/dist/hive/bin/hive --auxpath /opt/tez-autobuild-master/dist/hive/hcatalog/share/hcatalog/hive-hcatalog-core-1.3.0-SNAPSHOT.jar -i testbench.settings-4g-container 2>&1  | tee $query.log";
